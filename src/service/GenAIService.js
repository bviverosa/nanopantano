...
class GenAIService {
  async generateImage({ prompt, sesion_id, usuario_id }) {
    ...
    const options = {
      ...
      body: JSON.stringify({
        // 👈 Cambiado al modelo que te da 714 imágenes por tu dólar
        model: 'black-forest-labs/FLUX.1-schnell', 
        prompt: prompt.trim(),
        image_size: '1024x1024' // Puedes dejarlo en 1024x1024 para excelente calidad
      })
    };

    try {
      // 2. Ejecutar la petición HTTP a la API comercial de SiliconFlow
      ...
      // 3. Extraer la URL de la imagen generada según la estructura de SiliconFlow
      // Por estándar, responden en un arreglo llamado 'data' o 'images' con un objeto conteniendo 'url'
      ...
      // 4. Formatear el objeto final idéntico al de tu tabla sesion_mensaje
